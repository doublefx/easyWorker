...
import workers.*;
import workers.vo.WithEmbedVO;

use namespace thread_diagnostic;

public class ThreadUsingEmbedsTest extends SimpleThreadTestWithNoArgs {

    [Before]
    override public function setUp():void {
        const extraDependencies:Vector.<String> = new Vector.<String>();
        extraDependencies[0] = "workers.vo.WithEmbedVO";

        _thread = new Thread(WorkerUsingEmbeds, "workerUsingEmbeds", false, extraDependencies, loaderInfo);
    }

    [Test(description="Verify the name of the Thread")]
    override public function testName():void {
        assertEquals("Should be equal to 'workerUsingEmbeds'", _thread.name, "workerUsingEmbeds");
    }
    ...
    override public function testRunnableClassName():void {
        assertEquals(Thread(_thread).runnableClassName, "workers.WorkerUsingEmbeds");
    }
